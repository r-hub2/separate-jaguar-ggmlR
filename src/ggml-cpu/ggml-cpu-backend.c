...
// Thread power throttling definitions for Windows 10+
// These are not available in MinGW headers used by CRAN (rtools), so only enable for MSVC
// This is a performance optimization, not essential for functionality
#if defined(_MSC_VER)
#define GGML_USE_THREAD_POWER_THROTTLING
#endif

#ifdef GGML_USE_THREAD_POWER_THROTTLING
...
#endif
#if !defined(_THREAD_POWER_THROTTLING_STATE_DEFINED)
...
#endif
#endif // GGML_USE_THREAD_POWER_THROTTLING
