...
* **Multi-head attention** — `ggml_layer_attention()` and the shared-weights `ggml_attention()`, with `causal = TRUE` for GPT-style masking and `ggml_apply(list(query, context), attn)` for cross-attention. Input and output are both `c(seq_len, d_model)`, so layers stack; all heads run in one batched pass.
* **`ggml_layer_dense(time_distributed = TRUE)`** — one kernel per position, keeping the sequence axis (`c(seq_len, units)`). With attention, this completes a transformer block in the functional API.
* **MAE, Huber and binary cross-entropy losses** — `loss = "mae"`, `"huber"` (smooth L1, delta 1) or `"binary_crossentropy"` in `ggml_compile()`, and the matching `ggml_opt_loss_type_*()` constants. MAE and Huber resist outliers where MSE does not; binary CE expects probabilities (end the model in a sigmoid), one independent Bernoulli per output.
* **SSM / RWKV bindings** — `ggml_ssm_conv()`, `ggml_ssm_scan()` (Mamba), `ggml_rwkv_wkv6()`, `ggml_rwkv_wkv7()` and `ggml_gated_linear_attn()`. Each returns the sequence output and the final state in one packed tensor; `ggml_ssm_scan_output()` / `ggml_ssm_scan_state()` and `ggml_rwkv_output()` / `ggml_rwkv_state()` view either half. Inference only — none of these ops has a backward pass in ggml.
* **Multi-output training** — a functional model with several outputs now trains *all* of its heads; previously `ggml_fit()` silently optimized only the last one. Each output takes its own loss and weight via `ggml_compile(loss = list(policy = "categorical_crossentropy", value = "mse"), loss_weights = c(policy = 1, value = 0.5))`, minimizing `sum(loss_weights[i] * loss_i)`. Losses are matched to outputs by layer name (as in keras) or by position, `y` is passed as a list of matrices, and cross-entropy and regression heads may be mixed in one model. `model$history` reports per-head `train_<output>_loss` (plus `val_<output>_loss` when a validation split is used) and `ggml_evaluate()` a per-head `<output>_loss`, so a head that stops learning is visible rather than hidden inside the aggregate. Single-output models are unaffected: they build the same graph as before, node for node. Multi-output works on multi-input models too: a model with several inputs has no `ggml_opt_dataset` to batch from, so `ggml_fit()` drives its own batch loop and fills each head's labels tensor from that head's slice of the concatenated `y` — the per-head history and callback keys are the same ones the single-input path reports.
